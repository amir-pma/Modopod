  ...
  <div>
    ...
    <ul class="list-group mb-3">
      <li v-for="(item, index) in items" :key="index" class="list-group-item d-flex justify-content-between lh-condensed">
        <div>
          <h6 class="my-0">{{ item.variant.name }} <span class="text-muted">({{ item.quantity }})</span></h6>
          <small class="text-muted">{{ item.variant.colour }}</small>
        </div>
        <span class="text-muted">{{ item.variant.price * item.quantity | currency }}</span>
      </li>
