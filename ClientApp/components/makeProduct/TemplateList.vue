        ...
        <b-row :class="{designContainerNotUploaded:!showList, designContainerUploaded:showList}">
            ...
            <div v-show="showList">
                <b-tabs content-class="mt-3">
                    <b-tab title="فارسی" active>
                        <div v-show="showList" class="designInfoClass" style="direction:rtl">
                            ...
                            <label for="designMoreTag" class="designInfoTitleLabel">
                                افزودن تگ&nbsp;
                            </label>
                            <input placeholder="تگ های مورد نظر را انتخاب کنید" class="designInfoTitle" v-model="designInfoMoreTags" type="text" id="designMoreTag">
                    ...
                    <b-tab title="English">
                        <div v-show="showList" class="designInfoClass">
                            <div class="designInfoTitleLabelContainer">
                                <label for="designTitleEnglish" class="designInfoTitleLabelEng">
                                    Design Name&nbsp;
                                </label>
                            </div>
                            <input placeholder="Enter your design name" class="designInfoTitle" v-model="designInfoNameEng" type="text" id="designTitleEnglish">
                            <div class="designInfoTitleLabelContainer">
                                <label for="designTitle" class="designInfoTitleLabelEng">
                                    Tags&nbsp;
                                </label>
                            </div>
                            <select multiple="multiple" id="myMultiEnglish">
                                <option v-for="tag in tags" :key="tag.id+'eng'" :value="tag.name">{{tag.name}}</option>
                            </select>
                            <div class="designInfoTitleLabelContainer">
                                <label for="designMoreTagEng" class="designInfoTitleLabelEng">
                                    Add Your Tags&nbsp;
                                </label>
                            </div>
                            <input placeholder="Select tags" class="designInfoTitle" v-model="designInfoMoreTagsEng" type="text" id="designMoreTagEng">
                            <div class="designInfoTitleLabelContainer">
                                <label for="designDescriptionEng" class="designInfoTitleLabelEng">
                                    Design Description&nbsp;
                                </label>
                            </div>
                            <textarea rows="3" placeholder="Describe your design here" v-model="designInfoDescriptionEng" class="designInfoDescription" id="designDescriptionEng"></textarea>
                        </div>
                    </b-tab>
                </b-tabs>
            </div>
        ...
        <b-row v-if="showList" class="mt-4" style="width: 100%!important; margin:0px 30px 0px 0px;">
            <b-col v-show="template.display" class="mb-4" :cols="template.cols" v-for="template in templates" :key="template.template.id">
                <div v-if="template.isComponent">
                    <b-row>
                        <b-col cols="12" v-if="templatesData[template.number].previewLoadin-gForClosing" style=" height:600px; position:absolute; display:flex; justify-content:center; align-items:center; background-color: gray; opacity:0.6; z-index:10000">
                        ...
                        <b-col cols="8" style="display:flex; justify-content:center;">
                            <button type="button" class="btn disableButton btn-secondary m-auto" @click="createMockups(templatesData[template.number].id, template.number)" style="width: 150px; height: 40px">
                                <div style="display: flex; align-items: center; height: 100%; margin-top: -4px;">
                                    ساخت طرح ها
                                </div>
                                <b-spinner v-if="creatingMockups" style="width:6rem; height:6rem ;position:absolute; z-index:20;" variant="primary" label="Spinning"></b-spinner>
                            </button>
                            <transition name="fade" mode="out-in">
                                <div :id="'wholeTemplate' + template.number" style="width:600px; height:600px; display:flex; justify-content:center;">
                                    ...
                                    <div style="position: absolute; z-index: -1;">
                                        <div :id="'wholeTemplateMockup' + template.number" style="width:600px; height:600px; justify-content:center; position: relative;">
                                            <b-container @mouseup="updatePreview(null, template.number, false)" class="coverClass">
                                                <div class="colorMockup" :id="'colorPic' + template.number" v-if="Boolean(templatesData[template.number].designAddress)" :style="colorMakerStyle(template.number)">
                                                </div>
                                                <div :class="{noBorder:templatesData[template.number].previewLoading}" :style="templatePrintAreaStyle(template.number)">
                                                    <VueDragResize :isResizable="false" :w="templatesData[template.number].isRepeat ? 2222 : templatesData[template.number].picWidth" :h="templatesData[template.number].isRepeat ? 2222 : templatesData[template.number].picHeight" :x="templatesData[template.number].left" :y="templatesData[template.number].top" v-on:dragging="resize($event,template.number)" style="cursor:grab">
                                                        <div :id="'printAreaImage' + template.number" v-if="Boolean(templatesData[template.number].designAddress) && (templatesData[template.number].repeatType === 'bad')" style="width:100%; height: 100%; z-index: -1; position: relative; display:flex;">
                        ...
                        <b-col cols="12" class="row justify-content-center">
                            <div class="m-2" v-for="mockup in templatesData[template.number].specificMockups" v-if="templatesData[template.number].loadingMockup == false">
                                <img height="300" :src="templatesData[template.number].designPath + '/' + mockup.fileName" />
                                    
                                <b-button v-if="mockup.isSelected" @click="selectMockup(mockup, false)" class="enableButton m-auto mt-2">
                                    <div style="display:flex; align-items:center; height:100%; margin-top:-4px;">
                                        فعال
                                        <font-awesome-icon icon="check" class="ml-1" />
                                    </div>
                                </b-button>
                                <b-button v-else @click="selectMockup(mockup, true)" class="disableButton m-auto mt-2">
                                    <div style="display:flex; align-items:center; height:100%; margin-top:-4px;">
                                        انتخاب
            ...
            <b-col class="mb-4" :cols="template.cols" v-for="template in templates" :key="'FirstPreview' + template.template.id">
                <div v-if="template.isComponent">
                    ...
                    <b-row style="position:fixed; top:-1000px; right:-1000px;">
                        ...
                        <b-col cols="8" style="display:flex; justify-content:center; position: relative;">
                            <transition name="fade" mode="out-in">
                            ...
                            <div style="position: absolute; z-index: -1;">
                                <div :id="'wholeTemplateMockupFirstPreview' + template.number" style="width:600px; height:600px; justify-content:center; position: relative; z-index: -1;">
                                    <b-container @mouseup="updatePreview(null, template.number, false)" class="coverClass" :style="templateBGStyle(template.number)">
                                        <div class="colorMockup" :id="'colorPicFirstPreview' + template.number" v-if="Boolean(templatesData[template.number].designAddress)" :style="colorMakerStyle(template.number)">
                                        </div>
                                        <div :class="{noBorder:templatesData[template.number].previewLoading}" :style="templatePrintAreaStyle(template.number)">
                                            <VueDragResize :isResizable="false" :w="templatesData[template.number].isRepeat ? 2222 : templatesData[template.number].picWidth" :h="templatesData[template.number].isRepeat ? 2222 : templatesData[template.number].picHeight" :x="templatesData[template.number].left" :y="templatesData[template.number].top" v-on:dragging="resize($event,template.number)" style="cursor:grab">
                                                <div :id="'printAreaImageFirstPreview' + template.number" v-if="Boolean(templatesData[template.number].designAddress) && (templatesData[template.number].repeatType === 'bad')" style="width:100%; height: 100%; z-index: -1; position: relative; display:flex;">
    ...
    export default {
        ...
        data() {
            return {
                ...
                designInfoNameEng: "",
                designInfoMoreTagsEng: "",
                designInfoDescriptionEng: "",
                isFinalizing: false,
                creatingMockups: false
            };
        },
        props: {
            templates: {
        ...
        methods: {
            createMockups(id, index) {
                var vm = this;
                this.creatingMockups = true;
                var files = [];
                var fileForm = new FormData();
                vm.templatesData[index].loadingMockup = true
                var file = vm.dataUrlToFile(vm.templatesData[index].previewImageMockup, vm.actualTemplates[index].slug + ".png");
                fileForm.set("file", file, file.name);

                axios.post("/api/templates/mockup?uid=" + this.designPath + "&id=" + id, fileForm,
                    {
                        headers: {
                    ...
                    .then(response => {
                        vm.templatesData[index].loadingMockup = false
                        this.creatingMockups = false;
                    }).catch(response => {
                        vm.templatesData[index].loadingMockup = false
                        this.creatingMockups = false;
                    });
            ...
            getSelectedTags() {
                ...
                for (var i = 0; i < vm.tags.length; i++) {
                    if (vm.myDrop.isSelected(i) || vm.myDropEnglish.isSelected(i)) {
                        var toBePushed = {};
            ...
            finalizeOrder() {
                var vm = this;
                if (vm.designInfoName + vm.designInfoNameEng == "") {
                ...
                for (let index = 0; index < files.length; index++) {
                    axios.post(mediaApiUrl, files[index][1],
                        ...
                        .then(response => {
                            ...
                            postData.description = vm.designInfoDescription + ' ' + vm.designInfoDescriptionEng + ' ' + vm.designInfoMoreTags + " " + vm.designInfoMoreTagsEng;
                            postData.regular_price = (vm.templatesData[files[index][0]].price * (100 + vm.templatesData[files[index][0]].profit) / 100).toString();
                            postData.slug = vm.templatesData[files[index][0]].name + "-" + vm.designInfoName + " " + vm.designInfoNameEng;
                            postData.name = vm.templatesData[files[index][0]].name + " با طرح " + vm.designInfoName + " " + vm.designInfoNameEng;
                            postData.type = "simple";
                            postData.status = "pending";
                            //postData.tags = vm.getSelectedTags();
                            let selectedTags = vm.getSelectedTags();
                            postData.tags = vm.templatesData[files[index][0]].tags;
                            for (var i = 0; i < selectedTags.length; i++) {
                                let hasTag = postData.tags.find(t => t.id == selectedTags[i].id);
                                if (!hasTag) {
                                    postData.tags.push(selectedTags[i])
                                }
                            }
                            //postData.categories = [
                            //    {
                            ...
                            postData.images = [{
                                "src": "https://plugin.modopod.ir/" + response.data.designPath, //"https://modopod.ir/wp-content/uploads/2020/06/Mo-13.jpg",
                                "position": "0"
                            }]
                            for (var i = 0; i < vm.templatesData[files[index][0]].specificMockups.length; i++) {
                                if (vm.templatesData[files[index][0]].specificMockups[i].isSelected) {
                                    let image = {
                                        "src": "https://plugin.modopod.ir/" + vm.templatesData[files[index][0]].designPath + "/" + vm.templatesData[files[index][0]].specificMockups[i].fileName,
                                        "position": i + 1
                                    }
            ...
            updatePreview(template, num, isViewing) {
                ...
                //var node = document.getElementById('printAreaImage' + num);
                    ...
                    this.$nextTick(function () {
                        ...
                        var node2 = document.getElementById('wholeTemplateMockup' + num);
                        domtoimage.toPng(node2)
                            .then(function (dataUrl) {
                                vm.templatesData[num].previewImageMockup = dataUrl;
                            })
                            .catch(function (error) {
                                console.error('oops, something went wrong with domtoimage!', error);
                            });
            ...
            selectMockup(mockup, value) {
                mockup.isSelected = value;
            },
                            ...
                            { headers: { Authorization: "Basic eDN1M3ViNHAtcnc2dC13b3N4OjU0ejAtZXlqMDF1bnVobWpq" } }).then(response => {
                                ...
                                if (updatePrev) {
                                    this.$nextTick(function () {
                                        ...
                                        var node2 = document.getElementById('wholeTemplateMockup' + p);
                                        domtoimage.toPng(node2)
                                            .then(function (dataUrl) {
                                                vm.templatesData[p].previewImageMockup = dataUrl;
                                            })
                                            .catch(function (error) {
                                                console.error('oops, something went wrong with domtoimage!', error);
                                            });
                ...
                if (document.getElementById("widthDommy").innerHTML == "" || document.getElementById("heightDommy").innerHTML == "") {
                    ...
                    if (this.isEmpty) {
                        ...
                        if (this.isReplace) {
                            ...
                            var node2 = document.getElementById('wholeTemplateMockupFirstPreview' + num);
                            domtoimage.toPng(node2)
                                .then(function (dataUrl) {
                                    vm.templatesData[num].previewImageMockup = dataUrl;
                                })
            ...
            makeListReady() {
                ...
                for (p = 0; p < this.actualTemplates.length; p++) {
                    var toBePushed = {};
                    toBePushed.id = vm.actualTemplates[p].id;
                    ...
                    toBePushed.isAvailable = vm.actualTemplates[p].isAvailable;
                    let specificMockups = vm.actualTemplates[p].specificMockupListString ? vm.actualTemplates[p].specificMockupListString.split("|") : [];
                    toBePushed.specificMockups = [];
                    for (var i = 0; i < specificMockups.length; i++) {
                        toBePushed.specificMockups.push({ fileName: specificMockups[i], isSelected: false })
                    }

                    let splitedTags = vm.actualTemplates[p].tagsListString.split("|");
                    toBePushed.tags = [];
                    for (var i = 0; i < splitedTags.length; i++) {
                        toBePushed.tags.push({ id: splitedTags[i] })
                    }
                    let splitedCategories = vm.actualTemplates[p].categoriesListString.split("|");
                    toBePushed.categories = [];
                    for (var i = 0; i < splitedCategories.length; i++) {
                        toBePushed.categories.push({ id: splitedCategories[i] })
                    }
                    //toBePushed.tags = splitedTags.map(function (x) { return { id = x } });
                    //toBePushed.categories = splitedCategories.map(function(x){ return { id = x } });
        ...
        mounted() {
            var vm = this;
            axios.get('https://modopod.ir/wp-json/tag-groups/v1/terms', { headers: { Authorization: 'Bearer ' + localStorage.getItem('wpToken') } })
                .then(response => {
                    ...
                    this.$nextTick(function () {
                        ...
                        this.myDropEnglish = new drop({
                            selector: '#myMultiEnglish',
                            isEnglish: true
                        });
                    });
                }).catch(response => {
