    ...
    <div>
        <b-container fluid class="page">
            ...
            <b-row style="justify-content: center;">
                <template-list @changeView="view" @closeAll="closeAll"
...
<script>
    ...
    import TemplateList from "../components/makeProduct/TemplateList.vue";
            ...
            axios
              ...
              .then(
                axios.spread((templates, categories) => {
                  next(vm => vm.setData(templates.data.filter(e => e.isAvailable == true), categories.data));
                })
              );
        },
        beforeRouteUpdate(to, from, next) {
            axios.get("/api/templates", { params: to.query }).then(response => {
              this.templatesArray = response.data.filter(e => e.isAvailable == true);
              next();
            });
        },
        computed: {
