export const addProductToCart = (state, product) => {
    state.cart.push({ variant: product.variant, image: product.image, quantity: 1});
};
...
export const setProductQuantity = (state, payload) => {
  let cartItem = state.cart[payload.cartIndex];
  cartItem.quantity = payload.cartPayload.quantity;

    state.cart.splice(payload.cartIndex, 1, Object.assign({}, cartItem));
};
